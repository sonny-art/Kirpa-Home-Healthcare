    ...
    <div>
      <section style={s("background:linear-gradient(180deg,#EEF4FA 0%,#F8FBFD 100%);border-bottom:1px solid #E3E9F0")}>
        <div style={s("max-width:1200px;margin:0 auto;padding:clamp(34px,5vw,60px) 24px")}>
          ...
          <p style={s("color:#0b1016;font-size:1.32rem;line-height:1.6;margin:18px 0 0;max-width:62ch")}>Tell us a little about your loved one&apos;s needs and our team will reach out to talk through the right care plan. Prefer to talk now? Call <a href="tel:+17039395287" style={s("color:#15589A;font-weight:700;text-decoration:none")}>703-939-5287</a>.</p>
        </div>
      </section>
        ...
        <div style={s("max-width:1200px;margin:0 auto;padding:clamp(40px,5vw,72px) 24px;display:grid;grid-template-columns:repeat(auto-fit,minmax(300px,1fr));gap:clamp(28px,4vw,44px);align-items:start")}>
          ...
          <aside className="form-aside" style={s("flex:1;min-width:260px;display:grid;gap:18px;align-content:start")}>
            <div style={s("background:#0C2C52;color:#C5D4E6;border-radius:18px;padding:28px")}>
              <h3 style={s("color:#fff;font-size:1.32rem;font-weight:800;margin:0 0 18px")}>What happens next</h3>
              <ul style={s("list-style:none;padding:0;margin:0;display:grid;gap:18px")}>
                {STEPS.map((st) => (
                  <li key={st.h} style={s("display:flex;gap:14px;align-items:flex-start")}>
                    <span style={s(asideIco)}><svg width={20} height={20} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={2} strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">{st.icon}</svg></span>
                    <span><strong style={s("color:#fff;display:block;font-size:1.04rem")}>{st.h}</strong><span style={s("font-size:.96rem")}>{st.p}</span></span>
                  </li>
                ))}
              </ul>
            </div>
            <div style={s("background:#fff;border:1px solid #E3E9F0;border-radius:18px;padding:26px;text-align:center;box-shadow:0 1px 2px rgba(12,44,82,.04)")}>
              <p style={s("color:#283341;font-size:1rem;margin:0")}>Prefer to speak with someone?</p>
              <a href="tel:+17039395287" style={s("display:block;font-size:1.6rem;font-weight:800;color:#15589A;text-decoration:none;margin:6px 0 14px")}>703-939-5287</a>
              <a href="tel:+17039395287" className="hv-outline" style={s("display:inline-flex;width:100%;box-sizing:border-box;align-items:center;justify-content:center;gap:9px;background:#fff;color:#0C2C52;font-weight:700;font-size:1.04rem;padding:13px 20px;border-radius:12px;border:2px solid #D7DFE8;text-decoration:none")}><Phone size={18} />Call Now</a>
              <p style={s("color:#283341;font-size:.92rem;margin:14px 0 0")}>Care professionals available 24/7.</p>
            </div>
          </aside>
        </div>
      </section>
